...
class Siphon<State : Any, Change : Any, Action : Any>(
    ...
    private val actions: suspend (action: Action) -> Change,
    events: List<Flow<Change>> = emptyList()
) {
    ...
    private val actionFlow: Flow<Change> =
        actionRelay.asFlow().flatMapMerge { flow { actions(it) } }

    private val changes = merge(actionFlow, changeFlow, *events.toTypedArray())

    val state: Flow<State> = changes //TODO flatMapConcat here? To ensure thread safety?
